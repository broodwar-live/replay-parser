...
| **전투** | + weapons.dat | 지상+공중 무기, 데미지 타입, 실드, 스플래시 데미지, 다중 타격, 지형 고도 미스 |
| **기술 & 업그레이드** | + techdata.dat, upgrades.dat | 연구 비용/시간, 업그레이드 레벨 스케일링, 전투 보너스 적용 |
| **생산** | (포함됨) | 빌드 큐, 훈련 타이머, 자원 차감, 데이터 기반 서플라이 비용 |
| **건설 & 변태 타이머** | (포함됨) | 건물 건설 시간, 유닛/건물 변태 타이머 |
| **일꾼 채광** | (포함됨) | 일꾼이 미네랄/가스 수입 생성 |
| **주문 시전** | (포함됨) | 스톰, EMP, 이레디에이트, 플레이그 — 에너지 차감 + 범위 데미지 |
| **랠리 포인트** | (포함됨) | 훈련된 유닛이 건물 랠리 포인트로 자동 이동 |
| **수송** | (포함됨) | 드랍쉽, 셔틀, 오버로드 탑승/하차 |
| **에너지** | (포함됨) | 캐스터 에너지 추적 및 재생 (HT, 디파, 사이언스 베슬 등) |
| **실드 재생** | (포함됨) | 프로토스 실드 시간 경과 재생 |
| **유닛 충돌** | (포함됨) | 공간 해시 O(n) — 지상 유닛 겹침 방지, 건물 이동 차단 |
...
```sh
cargo test --workspace    # 289개 테스트
cargo test -p bw-engine   # 153개 유닛 + 6개 통합 테스트 (실제 리플레이 픽스처)
cargo test -p replay-core # 111개 유닛 + 12개 통합 + 7개 proptest
